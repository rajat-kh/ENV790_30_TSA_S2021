...
for (i in 1:ncols-1){
  print (ggplot(BR_complete, aes(x = BR_complete[,i+1] , y = BR_complete[,1] )) +
           ylab(colnames(BR_complete[i+1]))+
...
##Parametrize 
temp_ts <- ts(BR_complete[,3], start = c(1901,01), frequency = 12)
rainfall_ts <- ts(BR_complete[,2], start = c(1901,01), frequency = 12)
```
...
temp_ts_decomp <- stl(temp_ts, s.window = 12)
rainfall_ts_decomp <- stl(rainfall_ts, s.window = 12)
...
temp_df <- cbind(temp_ts_decomp[[1]], rainfall_ts_decomp[[1]])
