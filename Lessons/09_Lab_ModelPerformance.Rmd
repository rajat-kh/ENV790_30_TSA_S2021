...
MPE = sum((actual - forecast)/actual)*100/ n
...
model1 <- auto.arima(BR_Temp_ts_deseas, max.P = 0, max.D = 0, max.Q = 0)
checkresiduals(model1)

NAIVE_seas <- naive(BR_Temp_ts_deseas, h=12)
checkresiduals(NAIVE_seas)

MEAN_seas <- meanf(BR_Temp_ts_deseas, h = 12)
checkresiduals(MEAN_seas)

model1_for <- forecast(model1, h = 12)
ARIMA_scores <- accuracy(model1_for)
MEAN_scores <- accuracy(MEAN_seas)
NAIVE_scores <- accuracy(NAIVE_seas)

data <- data.frame(t(ARIMA_scores),  t(MEAN_scores,NAIVE_scores)
